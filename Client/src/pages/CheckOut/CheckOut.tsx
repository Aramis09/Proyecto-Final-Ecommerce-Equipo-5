...
export const CheckOut = () => {
  ...
  const deleteItem = (e: any) => {
    let itemData = items.filter(
      (i: any) => i.id === parseInt(e.target.value)
    )[0];
    let lessPrice;
    if (user !== undefined) {
      let genres = itemData.Genres.map((item: any) => item.name);
      if (genres.includes(todaysDiscount.genre)) {
        lessPrice =
          ((100 - todaysDiscount.discount) * parseFloat(itemData.price)) / 100;
            ...
            <div className={styles["items-container"]}>
              ...
              <div className={styles["card-container"]}>
                {listProductsShoppingCart.map((game: any, index) => {
                  if (user) {
                    let genres = game.Genres.map((item: any) => item.name);
                    if (genres.includes(todaysDiscount.genre)) {
                      return (
